...
public class KdUtils {
    ...
    public static String getJarDirPath(){
        return getJarDirPath(KdUtils.class);
    }
    public static String getJarDirPath(Class cls)
    {
    ...
    public static String long2StringHex(long inlong, int bit){
        ...
        while(result.length() < bytetype) {
            result = "0" + result;
        }
        result = result.substring(result.length() - bytetype);
        return "0x" + result;
    }
    ...
    public static boolean recursiveDelete(File fileOrDir)
    {
        if(fileOrDir.isDirectory())
        {
            ...
            {
                if(!KdUtils.recursiveDelete(innerFile))
                {
                    return false;
                }
            }
